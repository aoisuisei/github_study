...
class NaviAction:
  ...
  def __init__(self):
    rospack = rospkg.RosPack()#rosパッケージの照会
    rospack.list() #ROS環境のパッケージ名のリスト化
    self.path = rospack.get_path('task_programmer')#パッケージのファイルパス取得
    ## @brief 読み込まれたwaypointsのデータ
    self.wp_client = dynamic_reconfigure.client.Client("/waypoints_editor", timeout=30)#クライアントの設定
    ...
    self.tf_listener_ = tf.TransformListener()#座標変換

    self.wp_number_ = 0

  def get_wp(self):#ポイント編集機能のためにディレクトリを読み取り？
    ...
    with open(self.path + dir_name + '/' + file_name + '.yaml') as f:
        config = yaml.safe_load(f) or {}#yamlファイルの取り込み
  ...
  def set_goal(self,_number):
    ...
    self.ac.send_goal(self.goal)#目標位置への移動
    succeeded = self.ac.wait_for_result(rospy.Duration(600))#移動結果の取得,600ミリ秒で中止
  ...
  def set_via_point(self,_number):
    ...
    while not rospy.is_shutdown():
      ...
      elif (time.time() > timeout):#一時間経過して到達しないならタイムアウト失敗
  ...
  def set_velocity(self,_x,_y,_theta,_time):#速度指令
    ...
    while(time.time() < end_time  and not rospy.is_shutdown()):#指定時間経過するまで速度指令をpub
...
class MoveitCommand:
  ...
  def __init__(self):
    self.robot = moveit_commander.RobotCommander()#ロボットの運動学モデルやロボットの現在の関節状態などの情報を提供するクラスのインスタンス化
    self.scene = moveit_commander.PlanningSceneInterface()#周囲の世界に関するロボットの内部理解を取得、設定、更新するためのリモートインターフェースを提供するクラスのインスタンス化
    self.group = moveit_commander.MoveGroupCommander("torso")#プランニンググループ（関節のグループ）に対するインターフェース。モーション計画や実行に使用
    self.group.set_pose_reference_frame("base_link")#エンドエフェクタの位置姿勢を設定する際に用いられる基準座標を設定
    self.group.set_end_effector_link("body_link")#エンドエフェクタと見なすリンクの名前を設定します。

    self.robot_model = rospy.get_param("/seed_r7_ros_controller/robot_model_plugin")#ロボットのタイプを読み込み
  ...
  def set_lifter_position(self, _x, _z, _vel=1.0):
    ...
    target_pose = Pose()#Pose型のメッセージインスタンスの生成
    #位置姿勢を記述
    ...
    self.group.set_max_velocity_scaling_factor(_vel)#最大関節速度を下げるためのスケーリング係数を設定します。許可される値は0～1です。

    plan = self.group.plan()#動作計画の実行、戻り値はRobotTrajectory形式
    if type(plan) is tuple: # for noetic
      plan = plan[1]

    if(len(plan.joint_trajectory.points)==0):
      rospy.logwarn("can't be solved lifter ik")#ログの出力
      self.group.clear_pose_targets()#目標姿勢のクリア
      return 'aborted'
    else:
      rospy.loginfo("execute plan")
      self.group.execute(plan)#planの実行
...
class LIFTER(State):
  ...
  def __init__(self,_position):
    ...
    ## @brief リフターの絶対位置(x,y)[m]と速度スケーリング(0~1),float型に変換
...
class LOOP(State):
  ...
  def __init__(self,_count):
    ...
    sn.loop_count_array[_count[0]] = int(_count[1])#タスク分配列[0,0,0,0,…]のループ先タスク番号目にループ回数を代入
    self.count_array_ = copy.copy(sn.loop_count_array)#タスク分配列 例[0,0,n,0,…]を浅いコピー

    self.count_  = _count#[ループ先,回数]

    rospy.loginfo('init count is {} \t'.format(sn.loop_count_array))#置換フィールド{}に[0,0,n,0,…]を代入して表示

  ## @brief 遷移実行
  # @param userdata 前のステートから引き継がれた変数。今回は使用しない
  # @return ループがある場合はsucceeded、無い場合はaborted
  def execute(self, userdata):
    count = self.count_array_[self.count_[0]]#ループ回数を抽出
    count -= 1

    if(count > 0):
      rospy.loginfo('count {} times'.format(count))#ループ回数-1を表示
      self.count_array_[self.count_[0]] = count#[0,0,n-1,…]該当箇所を引き算する
      return 'succeeded'
    elif(count < -1):
      rospy.loginfo('infinity loop')#-1なら無限ループ

      return 'succeeded'
    else:
      rospy.loginfo('reset count')
      self.count_array_[self.count_[0]] = sn.loop_count_array[self.count_[0]]#現在のループ回数を初期指定回数にリセット
      return 'aborted'
...
class DISPLAY_BUTTON(State):
  ...
  def execute(self, userdata):
    ...
    if(self.command == "wp"):
      rospy.sleep(0.5)
      wp_config = na.get_wp()#waypointsリストの取得
      for i in range(len(wp_config)):#ポイント名称があるかをチェック
...
class Scenario:
  ...
  def __init__(self):
    ...
    if(len(sys.argv) != 2): file_name = "scenario.yaml"#シナリオファイル名の指定が無い時
  ...
  def read_task(self, _number):
    ...
    return rev['action']['task']#taskキーで値(タスク名)の取得
  ...
  def read_map(self, _number):
    rev = dict(self.scenario[_number])
    if( 'arg' in rev['action']):#argキーの存在確認
      return rev['action']['arg']
    else:
      return "None"
  ...
  def read_string_arg(self, _number):
    rev = dict(self.scenario[_number])
    return rev['action']['arg'].encode('utf_8')#文字コードに変換

#==================================
#if __name__=='__main__':.pyで実行時に行われる処理、import時には実行されない。
#==================================
if __name__ == '__main__':
  rospy.init_node('aero_scenario_node')#ノードの生成

  mc = MoveitCommand()#MoveitCommand()クラスのインスタンス化
  na = NaviAction()#NaviAction()クラスのインスタンス化
  ta = TaskAction()#TaskAction()クラスのインスタンス化
  sn = Scenario()#Scenario()クラスのインスタンス化
  screen = Screen()#Screen()クラスのインスタンス化、Screenはインポートしている
  motion = Motion()#Motion()クラスのインスタンス化
  btn = Button()#Button()クラスのインスタンス化
